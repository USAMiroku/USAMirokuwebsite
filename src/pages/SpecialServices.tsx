...
export default function SpecialServices() {
  ...
  const displayedServices = [...specialServices].sort(
    (a, b) => Number(b.slug === 'annual-ancestors') - Number(a.slug === 'annual-ancestors'),
  )
      ...
      <section className="px-6">
        <div className="mx-auto grid max-w-6xl gap-6 md:grid-cols-3">
          {displayedServices.map((service) => {
            ...
            const isFeatured = service.slug === 'annual-ancestors'
            return (
              <Link
                key={service.slug}
                to={`/special-services/${service.slug}`}
                className={`group flex min-h-[12rem] flex-col rounded-2xl border p-8 shadow-[0_24px_80px_rgba(15,23,42,0.08)] transition-all duration-300 hover:-translate-y-1 hover:border-sage-600/45 ${
                  isFeatured ? 'border-divine-gold/55 bg-[#fffaf0]' : 'border-[rgba(184,134,11,0.22)] bg-white'
                }`}
              >
                <span className="text-[10px] font-semibold uppercase tracking-[0.16em] text-sage-600">
                  {isFeatured ? copy.annualAncestorsLabel : copy.title}
                </span>
                ...
                <span className="mt-auto pt-6 text-[10px] font-semibold uppercase tracking-[0.14em] text-deep-slate transition-colors group-hover:text-sage-600">
                  {isFeatured ? copy.openAnnualAncestors : copy.openForm}
                </span>
              </Link>
            )
          })}
