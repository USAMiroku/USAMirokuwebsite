  ...
  return (
    <div className="space-y-12">
      <section className="rounded-[32px] bg-gradient-to-br from-emerald-50 via-white to-amber-50 px-6 py-10 sm:px-10">
        ...
        <h1 className="mt-2 text-3xl font-semibold text-slate-900 sm:text-4xl">{resource.title[language] ?? resource.title.en}</h1>
        <div className="mt-4">
      ...
      <section className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm">
        <p className="text-sm text-slate-700 whitespace-pre-line">{resource.content[language] ?? resource.content.en}</p>
      </section>
    </div>
  )
}
